...
def all_hours():
    ...
    last_updated=[q[0] for q in db.session.query(db.func.max(Hour.last_updated))]
    ...
    three_months = [q[0] for q in db.session.query(db.func.sum(Hour.hours_watched)).filter(Hour.month_end_at > three_months_ago).all()]

    six_months_ago = current_time -  relativedelta(months=+5)

    six_months = [q[0] for q in db.session.query(db.func.sum(Hour.hours_watched)).filter(Hour.month_end_at > six_months_ago).all()]

    nine_months_ago = current_time -  relativedelta(months=+8)

    nine_months = [q[0] for q in db.session.query(db.func.sum(Hour.hours_watched)).filter(Hour.month_end_at > nine_months_ago).all()]
    ...
    eleven_months_for_projection = [q[0] for q in db.session.query(db.func.sum(Hour.hours_watched)).filter(Hour.month_end_at > eleven_months_ago_for_projection).all()]

    twelve_months_ago = current_time -  relativedelta(months=+11)

    twelve_months = [q[0] for q in db.session.query(db.func.sum(Hour.hours_watched)).filter(Hour.month_end_at > twelve_months_ago).all()]

    s1 = [str(i) for i in last_month]

    result1 = str("".join(s1))

    variance1= round(int(result1)-333.33,2)
    ...
    variance3= round(int(result3)-1000)

    s6 = [str(i) for i in last_month]

    result6 = str("".join(s6))

    variance6= round(int(result1)-2000)

    s9 = [str(i) for i in last_month]

    result9 = str("".join(s9))

    variance9= round(int(result1)-3000)

    s12 = [str(i) for i in twelve_months]

    result12 = str("".join(s12))

    variance12= int(result12)-4000
